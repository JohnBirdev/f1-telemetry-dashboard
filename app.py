...
animation_performance = st.sidebar.slider(
    ...
    value=5,
    step=1,
    help="A animação fica mais rápida quando usamos menos frames/pontos."
)
...
def get_session(year: int, round_number: int):
    ...
    session, error = load_session(year, round_number)
...
if laps is None or laps.empty:
    st.warning("Não há dados disponíveis para essa sessão.")
    st.stop()
...
with tab1:
    ...
    fig_all = plot_all_race_laps_overview(laps, driver)
    ...
    try:
        ...
        x, y, t, sectors = interpolate_telemetry(
            telemetry,
            steps=animation_steps
        )

        fig_circuit = plot_animated_circuit(
            x,
            y,
            sectors,
            driver
...
with tab2:
    ...
    if driver1 != driver2:
        ...
        with m_left:
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;text-align:center;margin-bottom:8px;font-weight:600;opacity:0.8;">
            <div>Lap Time</div>
            <div>Top Speed</div>
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;text-align:center;font-size:22px;font-weight:800;margin-bottom:14px;">
        ...
        lap_delta = None
        if s1.get("lap_time_s") and s2.get("lap_time_s"):
            lap_delta = s2["lap_time_s"] - s1["lap_time_s"]

        s1_delta = None
        if s1.get("s1_s") and s2.get("s1_s"):
            s1_delta = s2["s1_s"] - s1["s1_s"]

        s2_delta = None
        if s1.get("s2_s") and s2.get("s2_s"):
            s2_delta = s2["s2_s"] - s1["s2_s"]

        s3_delta = None
        if s1.get("s3_s") and s2.get("s3_s"):
